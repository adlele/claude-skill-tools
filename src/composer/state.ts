...
import { die, warn } from "../shared/ui.js";
...
// Re-export die so existing imports from "./state" keep working
export { die } from "../shared/ui.js";
...
export function resolveSessionId(partial: string): string {
  ...
  if (!fs.existsSync(stateDir))
    die(`No session found matching '${partial}'`);
  const matches = fs
...
export function readState(sessionId: string): SessionState {
  ...
  try {
    ...
    die(`Corrupted session state file: ${file}`, [
...
export function listStateSessions(): SessionState[] {
  ...
  for (const f of fs.readdirSync(stateDir).filter(f => f.endsWith(".json"))) {
    try {
      ...
      warn(`Skipping corrupted state file: ${f}`);
    }
  }
  return results;
}
